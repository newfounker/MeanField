...
C this subroutine is taken from Numerical Recipes
      ...
      LOGICAL check
c GG:
      PARAMETER (NP=40,MAXITS=500,EPS=1.e-5,TOLF=1.e-6,TOLMIN=1.e-4,
     *TOLX=EPS,STPMX=100.) 
C NP = Mmaximum expected value of N
C MAXITS = maximum number of iterations
C EPS is close to machine precision
C TOLF sets the convergence criterion on function values
C TOLMIN sets the criterion for deciding whether spurious convergence to a minimum of fmin has occurred
C TOLX sets the convergence criterion of delta_x
C STPMX is the scaled maximum step length allowed in the searches 
c end GG
c      PARAMETER (NP=40,MAXITS=50,EPS=1.e-7,TOLF=1.e-8,TOLMIN=1.e-6,
c     *TOLX=EPS,STPMX=1000.)
